...
static void post_mp_init(void)
{
	...
	if (mp_run_on_all_cpus(vmx_configure, nullptr) != CB_SUCCESS)
		failure = true;

	if (CONFIG(SOC_INTEL_COMMON_BLOCK_SGX_ENABLE))
		if (mp_run_on_all_cpus(sgx_configure, nullptr) != CB_SUCCESS)
			failure = true;

	if (mp_run_on_all_cpus(fc_lock_configure, nullptr) != CB_SUCCESS)
		failure = true;
