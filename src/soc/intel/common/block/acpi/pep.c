			...
			acpigen_write_package(3);
			{
				...
				default:
					...
					printk(BIOS_WARNING,
						...
						dev->path.type);
					acpigen_emit_namestring(nullptr);
					break;
				}
...
static void (*lpi_s0_helpers[])(void *) = {
	nullptr,			/* enumerate functions (autogenerated) */
	acpi_lpi_get_constraints,/* get device constraints */
	nullptr,			/* get crash dump device */
...
void generate_acpi_power_engine_with_lpm(const struct soc_pmc_lpm *lpm)
{
	...
	size_t uuid_count = 1;
	struct dsm_uuid ids[] = {
		DSM_UUID(LPI_S0_HELPER_UUID, lpi_s0_helpers, ARRAY_SIZE(lpi_s0_helpers), nullptr),
...
{
	generate_acpi_power_engine_with_lpm(nullptr);
}
